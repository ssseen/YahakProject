...
def run_pipeline(image_path, x, y, stt_text=None, user_question="이 문제 좀 알려줘",
    ...
    x, y: find_finger_tip 등으로 이미 추출된 손끝 픽셀 좌표 (원본 이미지 기준).
    ...
    print("2. 유형 분류...")
    classification = classify_problem(ocr_result.get("ocr_text", ""), classification_override)
    ...
    if has_illustration:
        if _crop_illustration(cropped, ocr_result.get("illustration_bbox")) is not None:
